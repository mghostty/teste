...
const neutrals = ['slate', 'gray', 'zinc', 'neutral', 'stone', 'ash', 'sand', 'mist', 'lavender', 'sepia']
...
watch(() => appConfig.ui.colors.primary, (newVal) => localStorage.setItem('theme:primary', newVal))
watch(() => appConfig.ui.colors.neutral, (newVal) => localStorage.setItem('theme:neutral', newVal))
    ...
    >
      <template #leading>
        <UAvatar
          ...
          :class="[
            !collapsed ? '-left-1 scale-100 opacity-100' : 'scale-90 opacity-80',
          ]"
          :src="user.avatar.src"
          :alt="user.avatar.alt"
        />
